...
impl State {
    pub async fn new(window: Arc<Window>) -> anyhow::Result<Self> {
        ...
        let surface_caps = surface.get_capabilities(&adapater);
        ...
        let diffuse_texture = Texture::from_bytes(&device, &queue, diffuse_bytes, "tree").unwrap();
        ...
        let diffuse_bindgroup = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            label: Some("diffuse_bindgroup"),
        });

        let shader =
            device.create_shader_module(wgpu::include_wgsl!("../shaders/tex_shader.wgsl").into());
